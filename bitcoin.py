...
class Prediction :
  ...
  def create_model(self) :
    model = Sequential()
    # LSTMとは
    # LSTMはRNNを実現するために考案され、前の情報を上手く扱うことに特化した層を提供してくれる
...
if __name__ == "__main__":
  ...
  # データ準備
  data = None
  data_ = pandas.read_csv('./csv/bitcoin_log_1month.csv')
  data = data_ if (data is None) else pandas.concat([data, data_])
  data.columns = ['date', 'close', 'open', 'high', 'low', 'rate']
  data['date'] = pandas.to_datetime(data['date'], format='%Y年%m月%d日')
